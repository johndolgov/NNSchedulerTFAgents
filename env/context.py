...
class Context(py_environment.PyEnvironment, ABC):
    def __init__(self, state_n, nodes, run_times, tree, data):
        ...
        self._action_spec = tensor_spec.BoundedTensorSpec(
            shape=(), dtype=np.int32, minimum=0, maximum=len(self.actions)-1, name='action')

        self._observation_spec = tensor_spec.TensorSpec(
            shape=(64,), dtype=np.float32, name='observation')

        self._reward_spec = tensor_spec.TensorSpec(shape=(),
                                                   dtype=np.float32,
                                                   name='reward')
    ...
    def _step(self, action):
        act_t, act_n = self.actions[action]
        reward, end_time = self.make_action(act_t, act_n)
        ...
        if self._episode_ended:
            ...
            return ts.transition(self.state, reward=reward, discount=0.8)
    ...
    def update_state(self):
        self._episode_ended = len(self.scheduled) == self.n
...
if __name__ == '__main__':
    test_wfs, test_times, test_scores, test_size = wf_setup('Montage25')
    ttree, tdata, trun_times = test_wfs[0]
    environment = Context(5,[0,1,2,3,4],trun_times, ttree, tdata)
